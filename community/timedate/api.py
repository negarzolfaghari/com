...
class ListTimeAPIView(views.APIView):
    def post(self,request):
        ...
        if moorning_after_id==0:
            moorning=models.TakeTime.objects.filter(moorningOrafter=True)
        elif moorning_after_id==1:
            moorning=models.TakeTime.objects.filter(moorningOrafter=False)
        else:
            raise ValidationError(code="moorning_after_id is incorrect",datail="please enter correct moorning_after_id")
        moorning_after_ser=serializers.TaketimeSerializer(moorning,many=True)
